(ns portal.operation
  "OperationActor — one publish/feature/report/takedown operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (PortalCurator-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the PortalGovernor (:govern) and the
  ...
  decision to a human reviewer (trust & safety officer). The reviewer
  resumes with `{:approval {:status :approved}}` (or `:rejected`)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [portal.llm :as llm]
            [portal.policy :as policy]
            [portal.phase :as phase]
...
(defn build
  ...
  `portal.store/Store`).
  opts:
    :advisor      — a `portal.llm/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PortalCurator-LLM inference (the contained intelligence node) — proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (llm/-advise advisor store request)]
            {:proposal p :audit [(llm/trace request p)]})))

      ;; PortalGovernor — independent censor (separate system than the LLM).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        ...
                        :reason (or reason
                                    (cond (:takedown? verdict) :takedown-dispute
                                          (:sensitive? verdict) :sensitive-subject
                                          :else :low-confidence))
                        :phase ph
      ...
      ;; (trust & safety officer) resumes with :approval. Then route
      ;; commit/hold.
